...
const debug = require('debug')('debug')// eslint-disable-line
...
const getByRegionNameFromCawler = async (req, res, next) => {
  try {
    ...
    if (!(await City.create(cities)).length) { return next(new restify.InternalServerError('获取城市信息失败')) }
...
const put = async (req, res, next) => {
  try {
    ...
    if (deleteResult.result.ok !== 1) { return next(new restify.InternalServerError('更新城市列表失败')) }
